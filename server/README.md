...
`aiTrackedIdentities`, `aiRejected`. Панель «Аналитика» их не показывает — она
рисует только перечисленные по именам поля (`AnalyticsPanel.tsx`), лишние игнорирует, —
смотреть curl'ом. Счётчик отказов накопительный с момента старта процесса и обнуляется
рестартом. Ненулевые отказы при пустой посещаемости означают, что эндпоинты кто-то щупает.

Два следствия, о которых стоит помнить заранее:

- **Общий предохранитель превращает проблему трат в проблему доступности.** Пока он
  держит 600 в час, отказ получают все подряд, включая нормальных пользователей, а тот,
  кто долбит сервер, забирает освобождающиеся места по мере скольжения окна. При нынешней
  посещаемости это правильный размен, но выглядеть это будет как «сервер сломался».
- **Лимит считает запросы, а будущая квота продаёт генерации.** В плане платных функций
  речь про N бесплатных генераций в сутки, а здесь оба эндпоинта считаются в один общий
  счётчик. Параметр `cost` у `consume()` — заготовка под это, но квоте понадобится ещё
  и разделение по эндпоинтам. Учитывать при её реализации, иначе место придётся
  переписывать второй раз.
